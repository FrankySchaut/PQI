...
def append_run_summary(
    *,
    ...
    CSV.open("a", encoding="utf-8").write(
        f"{ts},{version},{test_cycle},{tests},{passed},{failed},{skipped},{duration_s:.2f},{notes.replace(',',';')}\n"
    )
    if not MD.exists():
        MD.write_text(
    ...
    MD.open("a", encoding="utf-8").write(
        f"| {ts} | {version} | {test_cycle} | {tests} | {passed} | {failed} | {skipped} | {duration_s:.2f} | {notes} |\n"
    )
